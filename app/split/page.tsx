...
function SplitPageClient() {
  ...
  const requestedUsageId = searchParams.get("usageId")?.trim() || null;
  ...
  }, [receipt, people, assignments, taxCents, tipCents]);
  const usageSnapshotSignature = useMemo(
      ...
      fallback={
        <Container maxWidth="lg" sx={{ minHeight: "100vh", py: { xs: 4, sm: 6 } }}>
          <Paper className="surface-panel" sx={{ borderRadius: 4, p: { xs: 3, sm: 4 } }}>
            <div className="rounded-2xl border border-slate-200/80 bg-white/80 px-5 py-10 text-center">
              <p className="step-kicker">Loading</p>
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Opening receipt splitter</h2>
            </div>
          </Paper>
        </Container>
      }
